...
async function load() {
  const state = await browser.runtime.sendMessage({ type: 'getSettings' });
...
byId('mode-block').addEventListener('click', async () => { await browser.runtime.sendMessage({ type: 'setMode', mode: 'block' }); load(); });
byId('mode-log').addEventListener('click', async () => { await browser.runtime.sendMessage({ type: 'setMode', mode: 'log' }); load(); });
byId('mode-allow').addEventListener('click', async () => { await browser.runtime.sendMessage({ type: 'setMode', mode: 'allow' }); load(); });
byId('notifications').addEventListener('change', async (e) => { await browser.runtime.sendMessage({ type: 'setNotifications', enabled: e.target.checked }); load(); });
byId('clear').addEventListener('click', async () => { await browser.runtime.sendMessage({ type: 'clearLogs' }); load(); });
